...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // 4. Cargar el logo si lo deseas en los PDFs
  loadLogo();
});
...
async function initUserAndSucursal() {
  ...
  if (!loggedInUsername) {
    alert("No hay usuario logueado, redirigiendo a login...");
  ...
  try {
    ...
    if (userSnapshot.empty) {
      alert("Usuario no encontrado en la base de datos. Inicia sesión de nuevo.");
      window.location.href = "login.html";
      return;
    }
    ...
    userPermissions = userDoc.permisos || {}; // Leemos los permisos si existen, si no, objeto vacío
  } catch (error) {
    console.error("Error al obtener sucursal/permisos del usuario:", error);
    alert("Error al obtener la sucursal del usuario. Revisa tu conexión o credenciales.");
  }
}
...
async function loadSucursalesForAdmin() {
  ...
  try {
    ...
    alert("Error al cargar sucursales para admin: " + error.message);
  }
}
...
function loadLogo() {
  ...
  img.onerror = function () {
    console.error("Error al cargar el logo.png");
    alert("Error al cargar el logo.png. Asegúrate de que la imagen esté en la carpeta correcta.");
  };
}
...
function createOrderCard(orderDocId, order, status) {
  ...
  // Construimos el HTML manualmente para decidir qué botones mostrar
  let cardHTML = `
  ...
  // Botones para status 'inProcess': Confirmar recepciones, marcar completado
  if (status === "inProcess") {
    // Permitir Confirmar Cantidades Recibidas / Marcar completado
    // SOLO si admin o canChangeStatus
  ...
  // Botones para status 'completed': Mostrar Pedido Recibido, Cambiar Estado
  if (status === "completed") {
    // Cambiar estado SOLO si admin o canChangeStatus
    cardHTML += `
  ...
  // Exportar pedido => se puede dejar a todos o solo admin (depende tu lógica)
  ...
  // Cambiar estado desde pending a inProcess => admin o canChangeStatus
  if (status === "pending") {
    if (userRole === "administrador" || userPermissions.canChangeStatus) {
      cardHTML += `
  ...
  // Generar Recibo Manual / Automático => Probablemente todos pueden, o decides
  cardHTML += `
  ...
  // Cargamos los recibos existentes
  ...
  try {
    ...
    if (!receiptsSnapshot.empty) {
      ...
      receiptsSnapshot.forEach((doc, index) => {
        ...
        `;
        // Mostrar botón "Eliminar Recibo" solo si admin o canDeleteReceipt
    ...
    alert("Error al cargar recibos: " + error.message);
  }
}

// ========================================================================
//           ELIMINAR RECIBO DE UN PEDIDO (CHECAR PERMISO EN BOTÓN)
// ========================================================================
async function deleteReceipt(orderId, receiptId) {
  if (confirm("¿Estás seguro de que deseas eliminar este recibo?")) {
    try {
      // Verificar una vez más, si quisieras, userRole o userPermissions.canDeleteReceipt
      await db
        .collection("orders")
        .doc(orderId)
        .collection("receipts")
...
async function deleteOrder(orderId) {
  if (confirm("¿Estás seguro de que deseas eliminar este pedido?")) {
    try {
      ...
      alert("Pedido y sus recibos eliminados exitosamente");
      reloadOrders();
    } catch (error) {
...
function confirmOrder(orderId) {
  // Verificar si se tienen permisos para cambiar estado (o userRole=admin)
  if (!(userRole === "administrador" || userPermissions.canChangeStatus)) {
    alert("No tienes permiso para confirmar recepción de este pedido.");
    return;
  }
...
async function saveConfirmedOrder() {
  ...
  try {
    ...
    alert("Cantidades recibidas y comentarios guardados exitosamente");
...
// ========================================================================
//         CAMBIAR ESTADO DEL PEDIDO MANUALMENTE (COMPLETADOS, ETC.)
// ========================================================================
function openChangeStatusModal(orderId) {
  // Verificar permiso para cambiar estado
  if (!(userRole === "administrador" || userPermissions.canChangeStatus)) {
    alert("No tienes permiso para cambiar el estado de este pedido.");
    return;
  }
...
async function changeOrderStatusManually(newStatus) {
  ...
  try {
    ...
    alert(`Estado del pedido cambiado a "${getStatusText(newStatus)}" exitosamente.`);
...
// Cambiar de pending a inProcess
async function changeOrderStatus(orderId, newStatus) {
  // Verificar permiso para cambiar estado
  if (!(userRole === "administrador" || userPermissions.canChangeStatus)) {
    alert("No tienes permiso para cambiar este pedido de estado.");
    return;
  }
  try {
    await db.collection("orders").doc(orderId).update({ status: newStatus });
...
// ========================================================================
//          GENERAR RECIBO MANUAL (SIN BLOQUEOS, O LO DECIDES)
// ========================================================================
function openGenerateReceiptModal(orderId) {
  ...
  db.collection("orders")
    ...
    .catch((error) => {
      ...
      alert("Error al obtener detalles del pedido: " + error.message);
    });
}
...
async function saveReceipt() {
  ...
  try {
    ...
    alert("Recibo manual guardado exitosamente.");
    ...
    console.error("Error al guardar el recibo manual:", error);
    alert("Error al guardar el recibo manual: " + error.message);
  }
}
...
async function loadAutomaticReceiptProducts(orderId) {
  try {
    ...
    console.error("Error al cargar productos para recibo automático:", error);
    alert("Error al cargar productos para recibo automático: " + error.message);
  }
}
...
async function saveAutomaticReceipt() {
  ...
  if (!receiptDate || !receiptDescription) {
    alert("Por favor, completa todos los campos correctamente (fecha y descripción).");
    return;
  }
  ...
  for (let i = 0; i < productDivs.length; i++) {
    ...
    if (checkbox.checked) {
      ...
      if (isNaN(receivedQuantity) || receivedQuantity < 0) {
        alert(`Cantidad recibida inválida para el producto "${productName}".`);
        return;
      }
      if (isNaN(price) || price < 0) {
        alert(`Precio inválido para el producto "${productName}".`);
        return;
      }
  ...
  if (selectedProducts.length === 0) {
    alert("No has seleccionado ningún producto para el recibo automático.");
    return;
  }
  ...
  try {
    ...
    alert("Recibo automático guardado exitosamente. Total calculado: Q" + calculatedTotal.toFixed(2));
    ...
    console.error("Error al guardar el recibo automático:", error);
    alert("Error al guardar el recibo automático: " + error.message);
  }
}
...
async function downloadReceipt(orderId, receiptId) {
  try {
    ...
    if (!logoBase64) {
      alert("El logo aún se está cargando. Intenta nuevamente en unos segundos.");
      return;
    }

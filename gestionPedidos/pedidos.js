...
function createOrderCard(orderDocId, order) {
  ...
  `;
  // Botón para mostrar detalles del pedido (en el modal se usará solo "Exportar Pedido como Imagen")
  html += `<button onclick="showOrderDetails('${orderDocId}')">Mostrar Pedido</button>`;
  
  // Elimina el botón "Editar Pedido" en la tarjeta para el admin
  if (userRole !== "administrador") {
  ...
  // Botón adicional en rojo para que el admin cambie el estado
  if (userRole === "administrador") {
...
/**********************************************************
 * showOrderDetails
 * (Se elimina el botón adicional de exportar; se usará
 * únicamente el botón "Exportar Pedido como Imagen" del modal)
 **********************************************************/
function showOrderDetails(orderId) {
  db.collection("orders").doc(orderId).get()
    .then(docSnap => {
      ...
      // Se asigna el ID del pedido al botón de "Exportar Pedido como Imagen"
...
function editOrder(orderDocId) {
  db.collection("orders").doc(orderDocId).get()
    .then(docSnap => {
      ...
      // Asignar el valor del destino en el select
...
async function saveEditedOrder() {
  try {
    ...
    // Se obtiene el destino del select (ahora se edita en una lista)
...
async function exportAsImageDirect(orderId) {
  try {
    ...
    const order = docSnap.data();
    const fileName = `Pedido_${order.providerName}_${order.orderId}_${order.orderDate}`;
    // Llama a la función que exporta usando el ticket oculto "orderDetailsForImage"
    exportAsImageTicket(order, fileName);
  } catch (error) { Swal.fire({ icon: "error", title: "Error", text: error.message }); }
}
function exportAsImage(order, fileName) {
  // Función original (no utilizada para exportar desde el modal "Mostrar Pedido")
...
/**********************************************************
 * exportAsImageTicket:
 * Función para exportar usando el ticket oculto "orderDetailsForImage"
 * (idéntico al de "Realizar Pedido", mostrando "Entrega en:" y "Fecha de Registro")
 **********************************************************/
function exportAsImageTicket(order, fileName) {
  // Usamos el contenedor "orderDetailsForImage"
  ...
  // En "Entrega en:" mostramos el destino (Bodega o Tienda)
  document.getElementById("imgDestination").textContent = order.destination;
  // En "Fecha de Registro:" usamos savedDate si existe; de lo contrario, orderDate
  ...
  // Mostrar temporalmente el ticket
...
/**********************************************************
 * Funciones para cambiar el estado (Modal "Cambiar Estado")
 **********************************************************/
...
async function changeOrderStatusManually(newStatus) {
  if (!currentOrderForStatusChange) {
    Swal.fire({ icon: "error", title: "Error", text: "No se ha seleccionado ningún pedido." });
    return;
  }
  try {
    await db.collection("orders").doc(currentOrderForStatusChange).update({ status: newStatus });
    Swal.fire({ icon: "success", title: "Estado actualizado", text: `El estado se cambió a ${newStatus}.` });
